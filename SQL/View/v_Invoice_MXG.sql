...
select
  ...
  case when P.CamCount = 0 then 'Ñòåêëî ' else 'Ñòåêëîïàêåò ' end +  
  ...
  case when P.bShpros <> 0 then ' ' + dbo.f_GetGPRasInfo(P.ID, P.bShpros) +   
                                ' ' + ltrim(str((select sum(LengReal) from RasShrink where idProject = P.ID))) + 'ìì'   
    else ''   
  end  
    + ', ' + ltrim(str(P.nCount)) + ' øò.' as GPNameStr,  
